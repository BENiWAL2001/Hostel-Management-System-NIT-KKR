        ...
        <div class="wrapper">
            ...
            <form action="" method="post" class="form" enctype="multipart/form-data">
                ...
                <div class="subject" style="border:none;">
                    <input type="file" name="mess_menu">
                </div>
...
if (isset($_POST['csubmit'])) {
    ...
    $gender = $_POST['gender'];
    if (isset($_FILES['mess_menu']['name'])) {
        $mess_menu = $_FILES['mess_menu']['name'];
        $source_path = $_FILES['mess_menu']['tmp_name'];
        if ($gender =="Male") {
            $destination_path = "../uploads/menu/Boys_hostel/" . $mess_menu;
            $upload = move_uploaded_file($source_path, $destination_path);
        } 
        else{
            $destination_path = "../uploads/menu/Girls_hostel/" . $mess_menu;
            $upload = move_uploaded_file($source_path, $destination_path);
        }
    }
    $hostel_id = explode('H', $hostel_no)[1];
    if ($gender =="Male") {
        $sql = "UPDATE mhostels SET mess_menu = '$mess_menu' WHERE hostel_id = $hostel_id";
    } else {
        $sql = "UPDATE fhostels SET mess_menu = '$mess_menu' WHERE hostel_id = $hostel_id";
    }
